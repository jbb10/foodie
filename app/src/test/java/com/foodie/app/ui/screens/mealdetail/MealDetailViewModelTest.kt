...
class MealDetailViewModelTest {
    ...
    @Test
    fun `SaveClicked should set isSaving to true during operation`() = runTest {
        // Given
        whenever(updateMealEntryUseCase(any(), any(), any(), any()))
            .thenReturn(Result.Success(Unit))
        ...
        val stateDuringSave = viewModel.uiState.value
        assertThat(stateDuringSave.isSaving).isTrue()

        advanceUntilIdle()

        // After completion
        val statAfterSave = viewModel.uiState.value
        assertThat(statAfterSave.isSaving).isFalse()
    }
